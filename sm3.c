...
void sm3_final8x(sm3_context *ctx,
                    unsigned char *out0,
                    unsigned char *out1,
                    unsigned char *out2,
                    unsigned char *out3,
...
void sm3_transform8x(sm3_context *ctx,
    ...
    u256 T[64];
    u256 Temp1,Temp2,Temp3,Temp4,Temp5;
    int j;


    for(j = 0; j < 16; j++)
		T[j] = _mm256_set_epi32(0x79CC4519,0x79CC4519,0x79CC4519,0x79CC4519,0x79CC4519,0x79CC4519,0x79CC4519,0x79CC4519);
	for(j =16; j < 64; j++)
		T[j] = _mm256_set_epi32(0x7A879D8A,0x7A879D8A,0x7A879D8A,0x7A879D8A,0x7A879D8A,0x7A879D8A,0x7A879D8A,0x7A879D8A);
...
#define SM3ROUND_AVX(A, B, C, D, E, F, G, H, j, w,w1) \
    SS1 =  ROTL32(ADD3_32(ROTL32(A,12),E,ROTL32(T[j],j)), 7);\
    ...
    SM3ROUND_AVX(s[0], s[1], s[2], s[3], s[4], s[5], s[6], s[7], 0, w[0],w1[0]);    
    SM3ROUND_AVX(s[7], s[0], s[1], s[2], s[3], s[4], s[5], s[6], 1, w[1],w1[1]);
    SM3ROUND_AVX(s[6], s[7], s[0], s[1], s[2], s[3], s[4], s[5], 2, w[2],w1[2]);
    SM3ROUND_AVX(s[5], s[6], s[7], s[0], s[1], s[2], s[3], s[4], 3, w[3],w1[3]);
    SM3ROUND_AVX(s[4], s[5], s[6], s[7], s[0], s[1], s[2], s[3], 4, w[4],w1[4]);
    SM3ROUND_AVX(s[3], s[4], s[5], s[6], s[7], s[0], s[1], s[2], 5, w[5],w1[5]);
    SM3ROUND_AVX(s[2], s[3], s[4], s[5], s[6], s[7], s[0], s[1], 6, w[6],w1[6]);
    SM3ROUND_AVX(s[1], s[2], s[3], s[4], s[5], s[6], s[7], s[0], 7, w[7],w1[7]);
    SM3ROUND_AVX(s[0], s[1], s[2], s[3], s[4], s[5], s[6], s[7], 8, w[8],w1[8]);
    SM3ROUND_AVX(s[7], s[0], s[1], s[2], s[3], s[4], s[5], s[6], 9, w[9],w1[9]);
    SM3ROUND_AVX(s[6], s[7], s[0], s[1], s[2], s[3], s[4], s[5], 10, w[10],w1[10]);
    SM3ROUND_AVX(s[5], s[6], s[7], s[0], s[1], s[2], s[3], s[4], 11, w[11],w1[11]);
    w[16]=XOR3(P1(XOR3(w[0],w[7],ROTL32(w[13],15))),ROTL32(w[3],7),w[10]);
    w1[12]=XOR(w[12],w[16]);  
    SM3ROUND_AVX(s[4], s[5], s[6], s[7], s[0], s[1], s[2], s[3], 12, w[12],w1[12]);
    w[17]=XOR3(P1(XOR3(w[1],w[8],ROTL32(w[14],15))),ROTL32(w[4],7),w[11]);
    w1[13]=XOR(w[13],w[17]);  
    SM3ROUND_AVX(s[3], s[4], s[5], s[6], s[7], s[0], s[1], s[2], 13, w[13],w1[13]);
    w[18]=XOR3(P1(XOR3(w[2],w[9],ROTL32(w[15],15))),ROTL32(w[5],7),w[12]);
    w1[14]=XOR(w[14],w[18]); 
    SM3ROUND_AVX(s[2], s[3], s[4], s[5], s[6], s[7], s[0], s[1], 14, w[14],w1[14]);
    w[19]=XOR3(P1(XOR3(w[3],w[10],ROTL32(w[16],15))),ROTL32(w[6],7),w[13]);
    w1[15]=XOR(w[15],w[19]);
    SM3ROUND_AVX(s[1], s[2], s[3], s[4], s[5], s[6], s[7], s[0], 15, w[15],w1[15]);
...
{
    sm3_context* ctx;
    /* 首先，我们需要初始化 msgblocks 为零，为了避免内存中的任何垃圾值引起错误。 */
    memset(ctx->msgblocks, 0, sizeof(ctx->msgblocks));
    /* 使用 memcpy 将输入拷贝到 msgblocks 中 */
    memcpy(ctx->msgblocks, input, ilen < sizeof(ctx->msgblocks) ? ilen : sizeof(ctx->msgblocks));
    sm3_starts( ctx );
    sm3_final8x(ctx,output,output+4,output+8,output+12,output+16,output+20,output+24,output+28);
